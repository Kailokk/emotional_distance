...
float lerp(float grayscaleValue, float pixelValue, float blendValue) {
	return grayscaleValue * blendValue * (pixelValue - grayscaleValue);
}
...
void main(void) {
	...
	gl_FragColor = vec4(mix(bit, sourcePixel.x, interpolationValue), mix(bit, sourcePixel.y, interpolationValue), mix(bit, sourcePixel.z, interpolationValue), 1);

	//gl_FragColor = vec4(lerp(bit, ditherPixel.x, interpolationValue), lerp(bit, ditherPixel.y, interpolationValue), lerp(bit, ditherPixel.z, interpolationValue), 1);
